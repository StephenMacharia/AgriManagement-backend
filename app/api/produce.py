...
from app.models.produce import Produce as ProduceModel
from app.schemas.user import User
from app.schemas.produce import Produce as ProduceSchema, ProduceCreate, ProduceUpdate
...
@router.post("/", response_model=ProduceSchema)
def create_produce(
    produce: ProduceCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(is_farmer)
):
    db_produce = ProduceModel(**produce.dict(), farmer_id=current_user.id)
    db.add(db_produce)
    db.commit()
...
@router.get("/", response_model=List[ProduceSchema])
def read_produce(
    skip: int = 0,
    limit: int = 100,
    category: str = None,
    ...
    query = db.query(ProduceModel).filter(ProduceModel.is_available == available)

    if category:
        query = query.filter(ProduceModel.category == category)
    if farmer_id:
        # Only admin can filter by other farmers
        if current_user.role != "admin" and current_user.id != farmer_id:
            raise HTTPException(status_code=403, detail="Not enough permissions")
        query = query.filter(ProduceModel.farmer_id == farmer_id)
    elif current_user.role == "farmer":
        # Farmers can only see their own produce by default
        query = query.filter(ProduceModel.farmer_id == current_user.id)

    produce = query.offset(skip).limit(limit).all()
    return produce


@router.get("/{produce_id}", response_model=ProduceSchema)
def read_produce_item(
    produce_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user)
):
    db_produce = db.query(ProduceModel).filter(ProduceModel.id == produce_id).first()
...
@router.put("/{produce_id}", response_model=ProduceSchema)
def update_produce(
    produce_id: int,
